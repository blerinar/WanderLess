...
if( ! defined( 'ILOVEWP_THEME_URL' ) ) {
	define( 'ILOVEWP_THEME_URL', 'https://www.ilovewp.com/themes/photozoom/' );
}
if( ! defined( 'ILOVEWP_VERSION' ) ) {
	define( 'ILOVEWP_VERSION', '1.0.0' );
}
...
function photozoom_setup() {
	...
	add_image_size( 'thumb-ilovewp-featured', 1100, 500, true );
	add_image_size( 'thumb-featured-page', 300, 200, true );
...
function photozoom_custom_sizes( $sizes ) {
	return array_merge( $sizes, array(
		'thumb-ilovewp-slideshow' => __( 'Featured Image: Slideshow Size', 'photozoom' ),
		'thumb-featured-page' => __( 'Featured Image: Page Thumbnail', 'photozoom' ),
...
function photozoom_js_scripts() {
	...
	// Theme stylesheet.
	wp_enqueue_style( 'photozoom-style', get_stylesheet_uri() );
	...
	if (! is_admin()) {
		...
		wp_register_script( 'photozoom-scripts', get_template_directory_uri() . '/js/photozoom.js', array( 'jquery' ), '20180502', true );

		/* Contains the strings used in our JavaScript file */
		$photozoomStrings = array (
			'slicknav_menu_home' => _x( 'Click for Menu', 'The main label for the expandable mobile menu', 'photozoom' )
		);

		wp_localize_script( 'photozoom-scripts', 'photozoomStrings', $photozoomStrings );
 ...
 */
function photozoom_wp_title( $title, $sep ) {
	global $paged, $page;
	...
	if ( $paged >= 2 || $page >= 2 ) {
		$title = "$title $sep " . sprintf( esc_html__( 'Page %s', 'photozoom' ), max( $paged, $page ) );
	}

	return $title;
}

add_filter( 'wp_title', 'photozoom_wp_title', 10, 2 );
				...
				<div class="comment-metadata">
					<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
						<time datetime="<?php comment_time( 'c' ); ?>">
							<?php printf( esc_html_x( '%1$s at %2$s', '1: date, 2: time', 'photozoom' ), get_comment_date(), get_comment_time() ); ?>
						</time>
					</a>
